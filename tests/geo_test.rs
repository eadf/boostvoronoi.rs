...
fn geo_conversion_test_32() -> Result<(), boostvoronoi::BvError> {
    ...
    let output = Builder::<I, F>::default()
        // `boostvoronoi::Point::from()` accepts `geo::Coord` and `geo::Point`
        .with_vertices([geo::Coord::from([3_i32, -3])].iter())?
        .with_vertices([geo::Point::new(4_i32, -3)].iter())?
        // `boostvoronoi::Line::from()` accepts `[geo::Coord;2]`, `[geo::Point;2]` and `geo::Line`
        .with_segments(
            [[
                geo::Coord::from([0_i32, 0]),
                geo::Coord::from([2, -7]),
            ]]
            .iter(),
        )?
    ...
    for v in output.vertices().iter().map(|v| v.get()) {
        // geo::Coord::from(&boostvoronoi::Vertex)
        let _geo_coordinate_from_ref = geo::Coord::<F>::from(&v);
        // geo::Coord::from(boostvoronoi::Vertex)
        let _geo_coordinate = geo::Coord::<F>::from(v.clone());
...
fn geo_conversion_test_64() -> Result<(), boostvoronoi::BvError> {
    ...
    let output = Builder::<I, F>::default()
        // `boostvoronoi::Point::from()` accepts `geo::Coord` and `geo::Point`
        .with_vertices([geo::Coord::from([3_i64, -3])].iter())?
        .with_vertices([geo::Point::new(4_i64, -3)].iter())?
        // `boostvoronoi::Line::from()` accepts `[geo::Coord;2]`, `[geo::Point;2]` and `geo::Line`
        .with_segments(
            [[
                geo::Coord::from([0_i64, 0]),
                geo::Coord::from([2, -7]),
            ]]
            .iter(),
        )?
