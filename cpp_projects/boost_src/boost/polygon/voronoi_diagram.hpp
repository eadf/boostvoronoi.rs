...
color_type color() const {
	return color_ >> BITS_SHIFT;
}
color_type raw_color() const {
	return color_;
}
...
std::pair<void*, void*> _insert_new_edge(
	...
	dbg_print_output();
	std::cout << "edge12:" << dbg_edge(edge12) << std::endl;
	std::cout << "edge23:" << dbg_edge(edge23) << std::endl;
...
void _build() {
	...
	std::cout << std::endl << "B4 Remove degenerate edges, size:" << edges_.size() << std::endl;
	dbg_print_output();
	...
	for (edge_iterator it = edges_.begin(); it != edges_.end(); it += 2) {
		...
		if (v1 && v2 && vertex_equality_predicate_(*v1, *v2)) {
			std::cout << "Removing edge:"; dbg(&(*it));
			std::cout << std::endl;
			remove_edge(&(*it));
		}
		else {
			if (it != last_edge) {
				std::cout << "last_edge="; dbg(&(*last_edge));
				std::cout << "       it="; dbg(&(*it));
				//dbg_edge();
				// some serious voodo going on here
	...
	auto edgesize_b4 =  edges_.size();
	edges_.erase(last_edge, edges_.end());
	if (edgesize_b4-edges_.size()>0) {
		std::cout << "Tail removed " << edgesize_b4-edges_.size() <<" edges." << std::endl;
		dbg_edge();
	}
	dbg_edge();
	...
	std::cout << std::endl << "Set up incident edge pointers " << std::endl;
	//dbg_edge();
	...
	auto vertexsize_b4 =  vertices_.size();
	vertices_.erase(last_vertex, vertices_.end());
	if (vertexsize_b4-vertices_.size()>0) {
		std::cout << "Tail removed " << vertexsize_b4-vertices_.size() <<" vertices." << std::endl;
	}
	dbg_print_output();
	//std::cout << std::endl;
	//dbg_edge();

	std::cout << std::endl << "Set up next/prev pointers for infinite edges." << std::endl;
	...
	if (vertices_.empty()) {
		...
		//   std::cout << "cell " << cell_it->source_index() << std::endl;
		//}
		...
		for (cell_iterator cell_it = cells_.begin();
			...
			std::cout << "cell si=" << cell_it->source_index() << std::endl;
			...
			//std::cout << "left "; dbg(left_edge);
			//std::cout << "right "; dbg(right_edge);
		}
	}
	//dbg_print_output();
	std::cout << std::endl;
...
void remove_edge(edge_type* edge) {
	...
	//std::cout << "edge1:" << dbg_edge(edge1) << std::endl;
	//std::cout << "edge2:" << dbg_edge(edge2) << std::endl;
	//std::cout << "edge1_rot_next:" << dbg_edge(edge1_rot_next) << std::endl;
	//std::cout << "edge2_rot_prev:" << dbg_edge(edge2_rot_prev) << std::endl;
...
void dbg(edge_type* edge) {
	...
	for (uint i=0; i<cells_.size(); i++) {
		m[(const void*)& cells_[i] ] = cells_[i].source_index();
	}
	for (uint i=0; i<vertices_.size(); i++) {
		m[(const void*)&vertices_[i]] = i;
	}
	for (uint i=0; i<edges_.size(); i++) {
		m[(const void*)&edges_[i]] = i;
	}

	std::cout << "edge " << o(edge)
			  << " cell:" << o(edge->cell())
			  << " v0:" << o(edge->vertex0())
...
void dbg_print_output() {
	...
	for (uint i=0; i<cells_.size(); i++) {
		m[(const void*)& cells_[i] ] = i;//cells_[i].source_index();
	}
	for (uint i=0; i<vertices_.size(); i++) {
		m[(const void*)&vertices_[i]] = i;
	}
	...
	for (uint i=0; i<cells_.size(); i++) {
		...
		std::cout << "Cell:" << i
				  ...
				  << " c:" << c->raw_color();
		if (c->contains_point()) std::cout << " point";
		if (c->contains_segment()) std::cout << " segment";
		std::cout << std::endl;
	}
	for (uint i=0; i<vertices_.size(); i++) {
		auto v = &vertices_[i];
		std::cout << "Vertex:" << i << "==" << o(v)
			      << " x:" << v->x()
			      << " y:" << v->y()
...
std::string dbg_edge(const void* edge) {
	std::unordered_map<const void*, int> m;
