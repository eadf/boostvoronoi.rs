...
template <typename T>
std::ostream& operator<<(std::ostream& os, const point_2d<T>& p)
{
    return os << "(" << debug_number_padding(p.x()) << "," << debug_number_padding(p.y()) << ")";
}
...
template <typename T>
std::ostream& operator<<(std::ostream& os, const site_event<T>& s)
{
/*	if (s.is_segment()) {
		if (s.is_inverse()) {
			os << s.point0() << "¿" << s.point1();
		}else {
			os << s.point0() << "-" << s.point1();
		}
	} else {
	...
	os << ",si:" << s.sorted_index()
	   << ",ii:" << s.initial_index()
	   << ",f:" << s.flags();
*/
	std::cout << "#" << s.sorted_index();
	if (s.is_segment()){
		std::cout << "(" << debug_number_padding(s.x0())
				  << "," << debug_number_padding(s.y0())
				  << ")" << (s.is_inverse()?"¿":"-")
				  << "(" << debug_number_padding(s.x1())
				  << "," << debug_number_padding(s.y1())
				  << ")";
		} else {
			std::cout << "("
			<< debug_number_padding(s.x0())
			<< ","
			<< debug_number_padding(s.y0()) << ")";
		}
	std::cout << ",ii:" << s.initial_index();
	std::cout << ",f:" << s.flags();
	return os;
}
...
template <typename T>
std::ostream& operator<<(std::ostream& os, const circle_event<T>& s)
{
	os << "CircleEvent(x:" << s.x() << " ,y:" << s.y() << " ,lx:" << s.lower_x();
	os << (s.is_active()?", active":", inactive")<<")";
	return os;
}
...
void pop() {
	...
	c_.pop();
	c_list_.erase(it);
	if (c_.size() != c_list_.size()) {
		std:: cout << "circle event mis-match!!!!!!!!!" << std::endl;
	}
}
