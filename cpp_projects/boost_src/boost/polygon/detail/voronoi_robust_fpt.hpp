...
floating_point_type fpv() const {
	return fpv_;
}
relative_error_type re() const {
	return re_;
}
relative_error_type ulp() const {
	return re_;
}
...
_fpt eval1_fake(_int* A, _int* B) {
	_fpt rv = eval1_real(A,B);
	if (false){
		std::cout << "eval1" << std::endl;
		std::cout.precision(0);
		std::cout << std::fixed << "A=[" << convert(A[0]).d() << "]" << std::endl;
		std::cout << "B=[" << convert(B[0]).d() << "]" << std::endl;
		std::cout.precision(5);
		std::cout << "rv==" << rv.d() << std::scientific << std::endl;
	}
	return rv;
}
...
_fpt eval2(_int* A, _int* B) {
	_fpt rv = eval2_real(A,B);
	std::cout << "eval2" << std::endl;
	std::cout.precision(0);

	std::cout << std::fixed << "A=[" << convert(A[0]).d() << ", "<< convert(A[1]).d() << "]" << std::endl;
	std::cout << "B=[" << convert(B[0]).d() << ", "<< convert(B[1]).d() << "]" << std::endl;
	std::cout.precision(5);
	std::cout << "rv==" << rv.d() << std::scientific << std::endl;
	return rv;
}
...
_fpt eval3(_int* A, _int* B) {
	_fpt rv = eval3_real(A,B);
	if (false){
		std::cout << "eval3" << std::endl;
		std::cout.precision(0);

		std::cout << std::fixed << "A=[" << convert(A[0]).d() << ", "<< convert(A[1]).d() << ", "<< convert(A[2]).d() << "]" << std::endl;
		std::cout << "B=[" << convert(B[0]).d() << ", "<< convert(B[1]).d() << ", "<< convert(B[2]).d() << "]" << std::endl;
		std::cout.precision(5);
		std::cout << "rv==" << rv.d() << std::scientific << std::endl;
	}
	return rv;
...
_fpt eval4(_int* A, _int* B) {
	_fpt rv = eval4_real(A,B);
	if (false){
		std::cout << "eval4" << std::endl;
		std::cout.precision(0);

		std::cout << std::fixed << "A=[" << convert(A[0]).d() << ", "<< convert(A[1]).d() << ", "<< convert(A[2]).d() << ", "<< convert(A[3]).d() << "]" << std::endl;
		std::cout << "B=[" << convert(B[0]).d() << ", "<< convert(B[1]).d() << ", "<< convert(B[2]).d() << ", "<< convert(B[3]).d() << "]" << std::endl;
		std::cout.precision(5);
		std::cout << "rv==" << rv.d() << std::scientific << std::endl;
	}
	return rv;
