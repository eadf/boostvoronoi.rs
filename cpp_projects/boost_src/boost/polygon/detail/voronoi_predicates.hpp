...
static fpt_type robust_cross_product(int_x2_type a1_,
	...
	uint_x2_type a1 = static_cast<uint_x2_type>(is_neg(a1_) ?
	                                            -a1_ :
	                                            a1_);
	uint_x2_type b1 = static_cast<uint_x2_type>(is_neg(b1_) ?
	                                            -b1_ :
	                                            b1_);
	uint_x2_type a2 = static_cast<uint_x2_type>(is_neg(a2_) ?
	                                            -a2_ :
	                                            a2_);
	uint_x2_type b2 = static_cast<uint_x2_type>(is_neg(b2_) ?
	                                            -b2_ :
	                                            b2_);
...
public:
	...
	static Orientation eval(const Point& point1,
		...
		auto cp = robust_cross_product(dx1, dy1, dx2, dy2);
		return eval(cp);
	}
} ot;
...
bool operator()(const site_type& lhs, const site_type& rhs) const {
	//std::cout << "lhs=(" << lhs.x0() << "," <<lhs.y0() << ") ";
	//std::cout << "rhs=(" << rhs.x0() << "," <<rhs.y0() << ")";
	//std::cout << std::endl;
...
bool operator()(const site_type& left_site,
	...
	bool rv = real_comparison(left_site, right_site, new_point);
	if (false) {std::cout << "distance_predicate L=" << left_site
			  << " R=" << right_site
			  << ", key=" << new_point
			  << "==" << (rv?"true":"false") << std::endl;}
   return rv;
}

bool real_comparison(const site_type& left_site,
	                const site_type& right_site,
	                const point_type& new_point) const {
...
bool pp(const site_type& left_site,
        const site_type& right_site,
        const point_type& new_point) const {
    bool rv = pp_real(left_site, right_site, new_point);
	if (false){
			std::cout << " pp( L=" << left_site
			  << ", R=" << right_site
			  << ", key=" << new_point
			  << ")==" << (rv?"true":"false") << std::endl;
			std::cout << "  assert_eq!(pp("
    		  << "&new_site("
    		  << left_site.x0() << "," << left_site.y0()
			  << "," << left_site.x1() << "," << left_site.y1()
			  << "," << left_site.initial_index()
			  << "," << left_site.sorted_index()
			  << "," << left_site.flags() << "),"
			  << "&new_site("
			  << right_site.x0() << "," << right_site.y0()
			  << "," << right_site.x1() << "," << right_site.y1()
			  << "," << right_site.initial_index()
			  << "," << right_site.sorted_index()
			  << "," << right_site.flags() << "),"
			  << "&new_point("
			  << new_point.x() << "," << new_point.y()
			  << ")),"
			  << (rv?"true":"false") << ");//distance_predicate" << std::endl;
	}
    return rv;
}

bool pp_real(const site_type& left_site,
        const site_type& right_site,
        const point_type& new_point) const {
...
bool ps(const site_type& left_site,
        const site_type& right_site,
        const point_type& new_point,
		bool reverse_order) const {
    bool rv = ps_real(left_site, right_site, new_point, reverse_order);
    if (false) {
    	std::cout << " ps(L=" << left_site
			  << ", R=" << right_site
			  << ", key=" << new_point
			  << ", reverse_order=" << (reverse_order?"true":"false")
			  << ")==" << (rv?"true":"false") << std::endl;
		std::cout << "  assert_eq!(ps("
	    		  << "&new_site("
	    		  << left_site.x0() << "," << left_site.y0()
				  << "," << left_site.x1() << "," << left_site.y1()
				  << "," << left_site.initial_index()
...
bool ps_real(const site_type& left_site, const site_type& right_site,
...
bool ss(const site_type& left_site,
        const site_type& right_site,
        const point_type& new_point) const {
    bool rv = ss_real(left_site, right_site, new_point);
	if (false) {
		std::cout << " ss( L=" << left_site
			  << ", R=" << right_site
			  << ", key=" << new_point
			  << ")==" << (rv?"true":"false") << std::endl;
		std::cout << "  assert_eq!(ss("
			  << "&new_site("
			  << left_site.x0() << "," << left_site.y0()
			  << "," << left_site.x1() << "," << left_site.y1()
			  << "," << left_site.initial_index()
...
bool ss_real(const site_type& left_site,
        const site_type& right_site,
        const point_type& new_point) const {
...
kPredicateResult fast_ps(
	...
	if (left_site.x0()==4 && left_site.y0()==3
		&& right_site.x0()==3 && right_site.y0()==1
		&& new_point.x()==5 && new_point.y()==4) {
		std::cout << " ps(L=" << left_site
				  << ", R=" << right_site
				  << ", key=" << new_point
				  << ", reverse_order=" << (reverse_order?"true":"false")
...
bool operator() (const node_type& node1,
   ...
   bool rv = real_comparison_(node1, node2);
   //std::cout << "node_comparison_predicate rv="
   //		     << (rv?"true":"false") << std::endl;
   return rv;
}
...
bool real_comparison_(const node_type& node1,
                 const node_type& node2) const {
...
void pss(const site_type& site1,
	...
	big_int_type sign = ((point_index == 2) ?
	                     1 :
	                     -1) *
	                    (is_neg(orientation) ?
	                     1 :
	...
	if (recompute_c_x || recompute_lower_x) {
		...
		if (recompute_lower_x) {
			cA[3] = orientation * (dx * dx + dy * dy) * (is_neg(temp) ?
			                                             -1 :
			                                             1);
...
bool lies_outside_vertical_segment(
	...
	fpt_type y0 = to_fpt(s.is_inverse() ?
	                     s.y1() :
	                     s.y0());
	fpt_type y1 = to_fpt(s.is_inverse() ?
	                     s.y0() :
	                     s.y1());
