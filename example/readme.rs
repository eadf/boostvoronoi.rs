...
fn main() -> Result<(), BvError> {
    ...
    let result = {
        ...
        // `with_vertices()` accepts iterators of anything that implements `Into<boostvoronoi::Point>`
        vb.with_vertices(p.iter())?;
        // `with_segments()` accepts iterators of anything that implements `Into<boostvoronoi::Line>`
