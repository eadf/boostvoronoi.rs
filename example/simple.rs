...
where
    ...
    F1: OutputType + Neg<Output = F1> + geo::GeoFloat,
    I2: BigIntType + Neg<Output = I2>,
    F2: BigFloatType + Neg<Output = F2>+ geo::GeoFloat,
{
    brect_: geo::Rect<F1>,
    ...
    F2: BigFloatType + Neg<Output = F2>+ geo::GeoFloat,
{
    pub fn new() -> Self {
        Self {
            brect_: geo::Rect::<F1>::new(
                geo::Point::<F1>::new(F1::from(0.0).unwrap(), F1::from(0.0).unwrap()),
                geo::Point::<F1>::new(
    ...
    pub fn build(&mut self) -> Result<String, BVError> {
        ...
        let mut point_data = Vec::<boostvoronoi::Point<I1>>::new();
        let mut segment_data = Vec::<boostvoronoi::Line<I1>>::new();
    ...
    fn read_data(&mut self) {
        ...
        let to_segments = |points: &[[i32; 4]]| {
            let mut rv = Vec::new();
            for p in points.iter() {
                rv.push(boostvoronoi::Line::<I1>::new(
        ...
        for it in self.vd_.edges().iter().enumerate() {
            ...
            if !self.vd_.edge_is_finite(Some(edge_id)).unwrap() {
                ...
                if edge.is_curved() {
                    ...
                    let linestring = geo::LineString::from(samples).simplify(&F1::from(1.0).unwrap());
                    samples = linestring.0.iter().map(|c|(c.x, c.y)).collect();
                }
            }
    ...
    fn clip_infinite_edge(
        ...
        if cell1.contains_point() && cell2.contains_point() {
            ...
            origin.0 = (p1.0 + p2.0) * Self::castf32_o(0.5);
            origin.1 = (p1.1 + p2.1) * Self::castf32_o(0.5);
        ...
        let koef = side / Self::max(direction.0.abs(), direction.1.abs());
    ...
    fn cast_point_io(value: &boostvoronoi::Point<I1>) -> (F1, F1) {
        (
             Self::cast_io(value.x),
             Self::cast_io(value.y),
            )
    }

    fn cast_coord_io(value: &boostvoronoi::Point<I1>) -> (F1, F1) {
        (
            Self::cast_io(value.x),
            Self::cast_io(value.y),
        )
    ...
    fn gl_vertex2f(i: usize, c: &(F1,F1)) {
        println!("#{}: x:{:.4}, y:{:.4}", i, c.0, c.1);
    }
}
